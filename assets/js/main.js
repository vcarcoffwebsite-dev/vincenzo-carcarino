...
(function () {
  ...
  async function loadDir(path) {
    try {
      ...
      // Netlify non serve directory listing — usiamo manifest
      return [];
    } catch { return []; }
  }
  ...
  // ===== FETCH CONTENT DIR =====
  // Su Netlify i file JSON sono nella cartella content/
  // Usiamo un manifest generato al build, oppure fetch diretto dei file noti
  async function fetchContentDir(dir) {
    try {
      // Prima proviamo a caricare il manifest (generato da build)
      const manifest = await loadJSON(`/content/${dir}/_manifest.json`);
      if (manifest && Array.isArray(manifest)) {
        const results = await Promise.all(manifest.map(f => loadJSON(`/content/${dir}/${f}`)));
