/* =============================================
   MAIN JS — Vincenzo Carcarino
   Legge i contenuti direttamente da GitHub API
   Zero build, zero crediti Netlify per i contenuti
   ============================================= */
...
(function () {
  ...
  // ===== FETCH DA GITHUB RAW (file singolo) =====
  ...
  // ===== FETCH LISTA FILE DA GITHUB API =====
  ...
  // ===== RISOLVI URL IMMAGINI =====
  // Le immagini caricate dal CMS sono in /static/uploads/ su GitHub
  function resolveImg(src) {
    if (!src) return '';
    if (src.startsWith('http')) return src;
    // Path relativo → GitHub raw
  ...
  function applySettings(cfg) {
    ...
    if (cfg.hero_image) {
      ...
      if (bg) bg.style.backgroundImage = `url(${resolveImg(cfg.hero_image)})`;
    }
  ...
  function buildLinks(links) {
    ...
    const navLi = document.getElementById('navLinksLi');
  ...
  async function loadMusica() {
    ...
    const grid    = document.getElementById('musicaGrid');
    if (!grid || !section) return;
    if (!items.length) { section.style.display = 'none'; hideNav('navMusicaLi'); return; }
    ...
    items.forEach(item => {
      ...
      if (item.tipo === 'youtube' && item.youtube_url) {
        ...
        const coverSrc = resolveImg(item.cover);
        const cover = coverSrc
          ? `<img class="musica-cover" src="${coverSrc}" alt="${item.title}" loading="lazy">`
        ...
        mediaHtml = cover + `<audio class="musica-player" controls src="${resolveImg(item.audio_file)}" preload="metadata"></audio>`;
      }
      card.innerHTML = `
        ${mediaHtml}
  ...
  async function loadEventi() {
    ...
    if (!futuri.length) { section.style.display = 'none'; hideNav('navEventiLi'); return; }
  ...
  async function loadDiscografia() {
    ...
    const section = document.getElementById('discografia');
    const grid    = document.getElementById('discografiaGrid');
    if (!grid || !section) return;
    if (!items.length) { section.style.display = 'none'; hideNav('navDiscografiaLi'); return; }
    items.sort((a,b) => (b.anno||0) - (a.anno||0));
    grid.innerHTML = '';
    items.forEach(item => {
      const coverSrc = resolveImg(item.copertina);
      const card = document.createElement('div');
      ...
      card.innerHTML = `
        ${coverSrc
          ? `<img class="disco-cover" src="${coverSrc}" alt="${item.title}" loading="lazy">`
        ...
        <div class="disco-tipo">${item.tipo||'Singolo'}</div>
        <div class="disco-title">${item.title||''}</div>
        ${item.anno ? `<div class="disco-anno">${item.anno}</div>` : ''}
        ${linksHtml ? `<div class="disco-links">${linksHtml}</div>` : ''}`;
      grid.appendChild(card);
    });
  }
  ...
  async function loadVideoGallery() {
    ...
    if (!grid || !section) return;
    if (!items.length) { section.style.display = 'none'; hideNav('navVideoLi'); return; }
    items.sort((a,b) => (a.order||99) - (b.order||99));
    grid.innerHTML = '';
    items.forEach(item => {
      const id = ytId(item.youtube_url);
      ...
      card.innerHTML = `
        <div class="video-embed">
          <iframe src="https://www.youtube.com/embed/${id}" allowfullscreen loading="lazy" title="${item.title||''}"></iframe>
        </div>
  ...
  async function loadStampa() {
    ...
    const section = document.getElementById('stampa');
    const grid    = document.getElementById('stampaGrid');
    if (!grid || !section) return;
    if (!items.length) { section.style.display = 'none'; hideNav('navStampaLi'); return; }
    ...
    items.forEach(item => {
      ...
      const logo = resolveImg(item.logo);
      const card = document.createElement('div');
      ...
      card.innerHTML = `
        ...
        <div class="stampa-meta">
          ${logo ? `<img class="stampa-logo" src="${logo}" alt="${item.testata}">` : ''}
  ...
  async function loadGallery() {
    ...
    if (!items.length) { section.style.display = 'none'; hideNav('navGalleryLi'); return; }
    items.sort((a,b) => (a.order||99) - (b.order||99));
  ...
  async function loadCollaboratori() {
    ...
    const section = document.getElementById('collaboratori');
    const grid    = document.getElementById('collaboratoriGrid');
    if (!grid || !section) return;
    if (!items.length) { section.style.display = 'none'; hideNav('navCollabLi'); return; }
    items.sort((a,b) => (a.order||99) - (b.order||99));
    grid.innerHTML = '';
    items.forEach(item => {
      const fotoSrc = resolveImg(item.foto);
      const card = document.createElement('div');
      card.className = 'collab-card fade-in';
      card.innerHTML = `
        ${fotoSrc
          ? `<img class="collab-foto" src="${fotoSrc}" alt="${item.title}" loading="lazy">`
  ...
  async function loadShop() {
    ...
    if (!available.length) { section.style.display = 'none'; hideNav('navShopLi'); return; }
  ...
  function initCarousel() {
    ...
    if (prev) prev.onclick = () => updateCarousel(currentSlide - 1);
    if (next) next.onclick = () => updateCarousel(currentSlide + 1);
  ...
  function initContactForm() {
    ...
    if (!form) return;
    const msgField  = document.getElementById('cfMessaggio');
    ...
    form.addEventListener('submit', async function (e) {
      ...
      const nome      = val('cfNome');
      const email     = val('cfEmail');
      const oggetto   = val('cfOggetto');
      const messaggio = val('cfMessaggio');
      let ok = true;
      ...
      if (messaggio.length < 10) { setErr('cfMessaggio','errMessaggio','Messaggio troppo breve (min. 10 caratteri).'); ok = false; }
      if (!ok) return;
      let hcaptcha_token = '';
      if (window.hcaptcha) {
      ...
      try {
        ...
        if (data.success) {
          show('formSuccess'); setText('successMsg', data.message || 'Messaggio inviato!');
          form.reset(); if (charCount) charCount.textContent = '0';
          if (window.hcaptcha) hcaptcha.reset();
        } else {
          show('formError'); setText('errorMsg', data.error || 'Errore nell\'invio.');
        }
      } catch {
        show('formError'); setText('errorMsg','Errore di connessione. Riprova.');
      }
      setBtnLoading(false);
    });
  }
  ...
  function clearFormErrors() {
    ['cfNome','cfEmail','cfOggetto','cfMessaggio'].forEach(id => {
      ...
      if (el && el.closest('.form-field')) el.closest('.form-field').classList.remove('has-error');
    });
  ...
  function getPlatformIcon(type) {
    const icons = {
      ...
      instagram: '<svg width="28" height="28" viewBox="0 0 24 24" fill="url(#ig_m)"><defs><linearGradient id="ig_m" x1="0%" y1="100%" x2="100%" y2="0%"><stop offset="0%" stop-color="#FFDC80"/><stop offset="50%" stop-color="#F56040"/><stop offset="100%" stop-color="#833AB4"/></linearGradient></defs><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>',
    };
    return icons[type] || '';
  }
  ...
  document.addEventListener('DOMContentLoaded', async function () {
    ...
    // Carica impostazioni
    const cfg = await loadJSON('/content/impostazioni.json');
    if (cfg) applySettings(cfg);
  ...
  function applySettings(cfg) {
    ...
    setText('heroFirst', first);
    setText('heroLast', last);
    setAll('.artist-name', nome);
    setAll('.nav-logo', nome);
    setAll('.footer-logo', nome);
    ...
    if (cfg.profile_image) {
      ...
      const img = document.getElementById('profileImg');
      if (ph) ph.style.display = 'none';
      if (img) { img.src = cfg.profile_image; img.style.display = 'block'; }
    }
    ...
    // Contatti
    if (cfg.contatti) {
      setText('contactEmail', cfg.contatti.email || '');
      const emailLink = document.getElementById('contactEmailLink');
      if (emailLink && cfg.contatti.email) {
        emailLink.href = 'mailto:' + cfg.contatti.email;
        emailLink.textContent = cfg.contatti.email;
  ...
  function buildLinks(links) {
    ...
    platforms.forEach(p => {
      ...
      a.style.setProperty('--platform-color', p.color);
      a.innerHTML = `
        <div class="platform-icon">${getPlatformIcon(p.icon)}</div>
  ...
  // ===== CARICA MUSICA =====
  async function loadMusica() {
    const items = await fetchContentDir('musica');
    const section = document.getElementById('musica');
    const grid = document.getElementById('musicaGrid');
    if (!grid || !section) return;

    const active = items.filter(i => i && i.title);
    if (active.length === 0) { section.style.display = 'none'; hideNav('navMusicaLi'); return; }

    active.sort((a,b) => (a.order||99) - (b.order||99));
    grid.innerHTML = '';
    ...
    active.forEach(item => {
      ...
      if (item.tipo === 'youtube' && item.youtube_url) {
        const id = ytId(item.youtube_url);
        if (id) mediaHtml = `<div class="musica-youtube-wrap"><iframe src="https://www.youtube.com/embed/${id}" allowfullscreen loading="lazy"></iframe></div>`;
      } else if (item.tipo === 'audio' && item.audio_file) {
        const cover = item.cover
          ? `<img class="musica-cover" src="${item.cover}" alt="${item.title}" loading="lazy">`
          : `<div class="musica-cover-placeholder"><svg width="48" height="48" fill="none" stroke="currentColor" stroke-width="1" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polygon points="10,8 16,12 10,16"/></svg></div>`;
        mediaHtml = cover + `<audio class="musica-player" controls src="${item.audio_file}" preload="metadata"></audio>`;
      }
  ...
  // ===== CARICA EVENTI =====
  async function loadEventi() {
    const items = await fetchContentDir('eventi');
    const section = document.getElementById('eventi');
    const grid = document.getElementById('eventiGrid');
    if (!grid || !section) return;

    const oggi = new Date(); oggi.setHours(0,0,0,0);
    const futuri = items.filter(i => i && i.data && new Date(i.data) >= oggi);
    if (futuri.length === 0) { section.style.display = 'none'; hideNav('navEventiLi'); return; }
    ...
    futuri.forEach(ev => {
      ...
      const giorno = String(d.getDate()).padStart(2,'0');
      const mese = d.toLocaleDateString('it-IT', { month: 'long', year: 'numeric' });
      const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(ev.indirizzo || '')}`;

      const card = document.createElement('div');
      card.className = 'evento-card fade-in';
      card.innerHTML = `
        ${ev.immagine ? `<img class="evento-img" src="${ev.immagine}" alt="${ev.title}" loading="lazy">` : ''}
        <div class="evento-body">
          <div class="evento-date-wrap">
            <div class="evento-giorno">${giorno}</div>
            <div class="evento-mese">${mese}</div>
          ...
          ${ev.note ? `<div class="evento-note">${ev.note}</div>` : ''}
          <div class="evento-actions">
            <a href="${mapsUrl}" target="_blank" rel="noopener" class="btn-map">
              <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
              Mappa
            </a>
            ${ev.ticket_url ? `<a href="${ev.ticket_url}" target="_blank" rel="noopener" class="btn-ticket">🎟 Biglietti</a>` : ''}
  ...
  // ===== CARICA DISCOGRAFIA =====
  async function loadDiscografia() {
    const items = await fetchContentDir('discografia');
    const section = document.getElementById('discografia');
    const grid = document.getElementById('discografiaGrid');
    if (!grid || !section) return;
    if (!items.length) { section.style.display = 'none'; hideNav('navDiscografiaLi'); return; }
    ...
    items.forEach(item => {
      const card = document.createElement('div');
      card.className = 'disco-card fade-in';
      const cover = item.copertina
        ? `<img class="disco-cover" src="${item.copertina}" alt="${item.title}" loading="lazy">`
        : `<div class="disco-cover-placeholder"><svg width="48" height="48" fill="none" stroke="currentColor" stroke-width="1" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg></div>`;
      const linksHtml = [
        item.spotify_url ? `<a href="${item.spotify_url}" target="_blank" class="disco-link">Spotify</a>` : '',
        item.youtube_url ? `<a href="${item.youtube_url}" target="_blank" class="disco-link">YouTube</a>` : '',
  ...
  // ===== CARICA VIDEO GALLERY =====
  async function loadVideoGallery() {
    const items = await fetchContentDir('video');
    const section = document.getElementById('video-gallery');
    const grid = document.getElementById('videoGrid');
    if (!grid || !section) return;
    if (!items.length) { section.style.display = 'none'; hideNav('navVideoLi'); return; }
  ...
  // ===== CARICA STAMPA =====
  async function loadStampa() {
    const items = await fetchContentDir('stampa');
    const section = document.getElementById('stampa');
    const grid = document.getElementById('stampaGrid');
    if (!grid || !section) return;
    if (!items.length) { section.style.display = 'none'; hideNav('navStampaLi'); return; }
    ...
    items.forEach(item => {
      ...
      card.innerHTML = `
        ...
        <div class="stampa-estratto">${item.estratto}</div>
        <div class="stampa-meta">
          ${item.logo ? `<img class="stampa-logo" src="${item.logo}" alt="${item.testata}">` : ''}
          <span class="stampa-fonte">${item.testata}</span>
          ${d ? `<span class="stampa-data">${d}</span>` : ''}
        </div>
        ${item.url ? `<a href="${item.url}" target="_blank" rel="noopener" class="stampa-link">Leggi articolo →</a>` : ''}
  ...
  // ===== CARICA GALLERY CAROSELLO =====
  let carouselPhotos = [];
  async function loadGallery() {
    const items = await fetchContentDir('gallery');
    const section = document.getElementById('gallery');
    const track = document.getElementById('carouselTrack');
    if (!track || !section) return;
    if (!items.length) { section.style.display = 'none'; hideNav('navGalleryLi'); return; }
    ...
    carouselPhotos.forEach(item => {
      const slide = document.createElement('div');
      slide.className = 'carousel-slide';
      slide.innerHTML = `<img src="${item.foto}" alt="${item.title || 'Gallery'}" loading="lazy">`;
      track.appendChild(slide);
    });
  ...
  async function loadCollaboratori() {
    const items = await fetchContentDir('collaboratori');
    const section = document.getElementById('collaboratori');
    const grid = document.getElementById('collaboratoriGrid');
    if (!grid || !section) return;
    if (!items.length) { section.style.display = 'none'; hideNav('navCollabLi'); return; }
    ...
    items.forEach(item => {
      ...
      const foto = item.foto
        ? `<img class="collab-foto" src="${item.foto}" alt="${item.title}" loading="lazy">`
        : `<div class="collab-foto-placeholder"><svg width="32" height="32" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>`;
      card.innerHTML = `
        ${foto}
        <div class="collab-nome">${item.title}</div>
  ...
  // ===== CARICA SHOP =====
  async function loadShop() {
    const items = await fetchContentDir('shop');
    const section = document.getElementById('shop');
    const grid = document.getElementById('shopGrid');
    if (!grid || !section) return;
    const available = items.filter(i => i && i.disponibile !== false);
    if (!available.length) { section.style.display = 'none'; hideNav('navShopLi'); return; }
    ...
    available.forEach(item => {
      ...
      const prezzo = parseFloat(item.prezzo || 0).toFixed(2).replace('.', ',');
      card.innerHTML = `
        <img class="shop-img" src="${item.foto}" alt="${item.title}" loading="lazy">
        <div class="shop-body">
          <div class="shop-tipo">${item.tipo || 'CD'}</div>
          <div class="shop-title">${item.title}</div>
          <div class="shop-desc">${item.descrizione || ''}</div>
  ...
  // ===== FETCH CONTENT DIR =====
  async function fetchContentDir(dir) {
    try {
      const manifest = await loadJSON(`/data/${dir}/_manifest.json`);
      if (!manifest || !Array.isArray(manifest) || manifest.length === 0) return [];
      const results = await Promise.all(
        manifest.map(f => loadJSON(`/data/${dir}/${f}`))
      );
      return results.filter(Boolean);
    } catch (e) {
      console.warn(`Errore caricamento ${dir}:`, e);
      return [];
    }
  }
  ...
  function updateCarousel(idx) {
    ...
    const max = Math.max(0, carouselPhotos.length - slidesPerView);
    currentSlide = Math.max(0, Math.min(idx, max));
  ...
  function buildDots() {
    ...
    if (!dotsEl) return;
    slidesPerView = getSlidesPerView();
    const count = Math.max(1, carouselPhotos.length - slidesPerView + 1);
    dotsEl.innerHTML = '';
    for (let i = 0; i < count; i++) {
      const btn = document.createElement('button');
      btn.className = 'carousel-dot' + (i === 0 ? ' active' : '');
  ...
  function initContactForm() {
    ...
    form.addEventListener('submit', async function (e) {
      ...
      try {
        ...
        if (data.success) {
          show('formSuccess');
          setText('successMsg', data.message || 'Messaggio inviato!');
          form.reset();
          if (charCount) charCount.textContent = '0';
          if (window.hcaptcha) hcaptcha.reset();
        } else {
          show('formError');
          setText('errorMsg', data.error || 'Errore nell\'invio.');
        }
      } catch {
        show('formError');
        setText('errorMsg', 'Errore di connessione. Riprova.');
      }
      setBtnLoading(false);
    });
  }
